...
class Settings(BaseModelWithoutExtraKeys):
    ...
    section_order: list[str] | None = pydantic.Field(
        default=None,
        title="Section Order",
        description=(
            "Override the order of sections in the output. List section titles"
            " (e.g., ['Skills', 'Experience', 'Education']). Sections not listed"
            " appear after ordered ones in their original order. Defaults to the"
            " order in the YAML input."
        ),
    )
