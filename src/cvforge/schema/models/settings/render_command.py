...
- OUTPUT_FOLDER: The output folder path (e.g., cvforge_output)
...
- NAME_IN_TITLE_SNAKE_CASE: The name of the CV owner in title snake case (e.g., John_Doe)
...
class RenderCommand(BaseModelWithoutExtraKeys):
    output_folder: PlannedPathRelativeToInput = pydantic.Field(
        default=pathlib.Path("cvforge_output"),
        description=(
            "Base output folder for all generated files. The default value is"
            " `cvforge_output`. Referenced as `OUTPUT_FOLDER` in output path"
            " defaults.\n\n"
        ),
    )
