...
from .run_cvforge import collect_input_file_paths, run_cvforge
...
@app.command(
    name="render",
    help=(
        "Render a YAML input file. Example: [yellow]cvforge render"
        " John_Doe_CV.yaml[/yellow]. Details: [cyan]cvforge render --help[/cyan]"
    ),
    ...
    with ProgressPanel(quiet=quiet) as progress_panel:
        if watch:
            run_function_if_files_change(
                ...
                lambda: run_cvforge(input_file_path, progress_panel, **arguments),
            )
        else:
            run_cvforge(input_file_path, progress_panel, **arguments)
