...
def generate_typst(cvforge_model: RenderCVModel) -> pathlib.Path | None:
    ...
    Args:
        cvforge_model: Validated CV model with content and design.

    Returns:
        Path to generated Typst file, or None if generation disabled.
    """
    if cvforge_model.settings.render_command.dont_generate_typst:
        return None
    typst_path = resolve_cvforge_file_path(
        cvforge_model, cvforge_model.settings.render_command.typst_path
    )
    typst_contents = render_full_template(cvforge_model, "typst")
