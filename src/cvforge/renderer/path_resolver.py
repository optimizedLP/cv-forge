...
def resolve_cvforge_file_path(
    cvforge_model: RenderCVModel, file_path: pathlib.Path
) -> pathlib.Path:
    ...
    Example:
        ```py
        # Given model with name="John Doe", year=2025, output_folder=cvforge_output
        path = resolve_cvforge_file_path(
            model,
            pathlib.Path("/cv/OUTPUT_FOLDER/NAME_IN_LOWER_SNAKE_CASE_CV_YEAR.pdf"),
        )
        # Returns: pathlib.Path("/cv/cvforge_output/john_doe_CV_2025.pdf")
        ```

    Args:
        cvforge_model: CV model containing name, date, and output folder for
            substitution.
    ...
    """
    output_folder = cvforge_model.settings.render_command.output_folder
    ...
    file_path_placeholders: dict[str, str] = {
        **build_date_placeholders(current_date, locale=cvforge_model.locale),
        **build_name_variants(cvforge_model.cv.name),
    }
