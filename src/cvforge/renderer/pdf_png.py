...
def generate_pdf(
    cvforge_model: RenderCVModel, typst_path: pathlib.Path | None
) -> pathlib.Path | None:
    ...
    Args:
        cvforge_model: CV model for path resolution and photo handling.
        typst_path: Path to Typst source file to compile.

    Returns:
        Path to generated PDF file, or None if generation disabled.
    """
    if cvforge_model.settings.render_command.dont_generate_pdf or typst_path is None:
        return None
    pdf_path = resolve_cvforge_file_path(
        cvforge_model, cvforge_model.settings.render_command.pdf_path
    )
    typst_compiler = get_typst_compiler(
        cvforge_model._input_file_path, typst_path.parent
    )
    copy_photo_next_to_typst_file(cvforge_model, typst_path)
...
def generate_png(
    ...
    """
    if cvforge_model.settings.render_command.dont_generate_png or typst_path is None:
        return None
    png_path = resolve_cvforge_file_path(
        cvforge_model, cvforge_model.settings.render_command.png_path
    )
...
def copy_photo_next_to_typst_file(
    cvforge_model: RenderCVModel, typst_path: pathlib.Path
) -> None:
    ...
    Args:
        cvforge_model: CV model containing photo path.
        typst_path: Path to Typst source file.
    """
    photo_path = cvforge_model.cv.photo
...
def get_package_path() -> pathlib.Path:
    ...
    Why:
        Bundled Typst packages (cvforge, fontawesome) are shipped inside the
    ...
    """
    temp_dir = pathlib.Path(tempfile.mkdtemp(prefix="cvforge-pkg-"))
