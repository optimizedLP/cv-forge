...
// State to hold cvforge configuration for use by components
#let cvforge-config = state("cvforge-config", (:))
...
#let directional-inset(start: 0cm, end: 0cm) = {
  let is-rtl = cvforge-config.get().at("is-rtl")
  if is-rtl {
