...
def render_full_template(
    ...
    Example:
        ```py
        typst_document = render_full_template(cvforge_model, "typst")
        # Returns complete .typ file with preamble, header, and all sections

        markdown_document = render_full_template(cvforge_model, "markdown")
        # Returns complete .md file with header and all sections
        ```

    Args:
        cvforge_model: CV model to render.
    ...
    download_photo_from_url(cvforge_model)
    cvforge_model = process_model(cvforge_model, file_type)
    ...
    for cvforge_section in cvforge_model.cv.cvforge_sections:
        section_beginning = render_single_template(
            file_type,
            f"SectionBeginning.j2.{extension}",
            cvforge_model,
            section_title=cvforge_section.title,
            snake_case_section_title=cvforge_section.snake_case_title,
            entry_type=cvforge_section.entry_type,
        )
        ...
        entry_codes = []
        for entry in cvforge_section.entries:
            entry_code = render_single_template(
                file_type,
                f"entries/{cvforge_section.entry_type}.j2.{extension}",
                cvforge_model,
                entry=entry,
            )
...
def render_html(cvforge_model: RenderCVModel, markdown: str) -> str:
    ...
    Example:
        ```py
        markdown_content = render_full_template(cvforge_model, "markdown")
        html_document = render_html(cvforge_model, markdown_content)
        # Returns complete HTML with <head>, CSS, and converted Markdown body
        ```

    Args:
        cvforge_model: CV model for template context.
    ...
    return render_single_template(
        "html", "Full.html", cvforge_model, html_body=html_body
    )
...
def render_single_template(
    ...
    **kwargs,
    ...
    Example:
        ```py
        header = render_single_template("typst", "Header.j2.typ", cvforge_model)
    ...
    Args:
        ...
        cvforge_model: CV model providing template context.

    Returns:
        Rendered template as string.
    """
    jinja2_environment = get_jinja2_environment(cvforge_model._input_file_path)
    template = None
    if file_type == "typst":
        ...
        with contextlib.suppress(jinja2.TemplateNotFound):
            template = jinja2_environment.get_template(
                f"{cvforge_model.design.theme}/{relative_template_path}"
            )
    ...
    return template.render(
        cv=cvforge_model.cv,
        design=cvforge_model.design,
        locale=cvforge_model.locale,
        settings=cvforge_model.settings,
        **kwargs,
    )
