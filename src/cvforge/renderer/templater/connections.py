...
def compute_connections(
    ...
    Args:
        cvforge_model: CV model with contact information.
...
def parse_connections(cvforge_model: RenderCVModel) -> list[Connection]:
    ...
    for key in cvforge_model.cv._key_order:
        match key:
            case "email":
                emails = cvforge_model.cv.email
                ...
                for phone in phones:
                    ...
                    body = phonenumbers.format_number(
                        ...
                        getattr(
                            ...
                            cvforge_model.design.header.connections.phone_number_format.upper(),
                        ),
                    )
                    connections.append(
                        Connection(
            ...
            case "website":
                websites = cvforge_model.cv.website
                if not websites:
            ...
            case "location":
                url = None
                body = str(cvforge_model.cv.location)
                connections.append(
                    Connection(
            ...
            case "social_networks":
                if cvforge_model.cv.social_networks is None:
                    raise RenderCVInternalError(
                        "social_networks key present but value is None"
                    )
                for social_network in cvforge_model.cv.social_networks:
                    url = social_network.url
                    if cvforge_model.design.header.connections.display_urls_instead_of_usernames:
                        body = clean_url(url)
                    else:
            ...
            case "custom_connections":
                if cvforge_model.cv.custom_connections is None:
                    raise RenderCVInternalError(
                        "custom_connections key present but value is None"
                    )
                for custom_connection in cvforge_model.cv.custom_connections:
                    url = (
...
def compute_connections_for_typst(cvforge_model: RenderCVModel) -> list[str]:
    ...
    Args:
        cvforge_model: CV model with contact information and design settings.

    Returns:
        List of Typst-formatted connection strings ready for template insertion.
    """
    connections = parse_connections(cvforge_model)

    show_icon = cvforge_model.design.header.connections.show_icons
    hyperlink = cvforge_model.design.header.connections.hyperlink
...
def compute_connections_for_markdown(cvforge_model: RenderCVModel) -> list[str]:
