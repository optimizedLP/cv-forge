...
def download_photo_from_url(cvforge_model: RenderCVModel) -> None:
    ...
    Args:
        cvforge_model: CV model whose photo URL will be downloaded in-place.
    """
    if cvforge_model.cv.photo is None or isinstance(
        cvforge_model.cv.photo, pathlib.Path
    ):
        return

    url_str = str(cvforge_model.cv.photo)
    ...
    output_dir = cvforge_model.settings.render_command.output_folder
    ...
    cvforge_model.cv.photo = destination


def process_model(
    cvforge_model: RenderCVModel, file_type: Literal["typst", "markdown"]
) -> RenderCVModel:
    ...
    Args:
        cvforge_model: Validated CV model.
        file_type: Target format for format-specific processors.

    Returns:
        Processed model ready for templates.
    """
    cvforge_model = cvforge_model.model_copy(deep=True)

    string_processors: list[Callable[[str], str]] = [
        lambda string: make_keywords_bold(string, cvforge_model.settings.bold_keywords)
    ]
    if file_type == "typst":
        string_processors.extend([markdown_to_typst])

    cvforge_model.cv._plain_name = cvforge_model.cv.name
    cvforge_model.cv.name = apply_string_processors(
        cvforge_model.cv.name, string_processors
    )
    cvforge_model.cv.headline = apply_string_processors(
        cvforge_model.cv.headline, string_processors
    )
    cvforge_model.cv._connections = compute_connections(cvforge_model, file_type)
    cvforge_model.cv._top_note = render_top_note_template(
        cvforge_model.design.templates.top_note,
        locale=cvforge_model.locale,
        current_date=cvforge_model.settings._resolved_current_date,
        name=cvforge_model.cv.name,
        single_date_template=cvforge_model.design.templates.single_date,
        string_processors=string_processors,
    )

    cvforge_model.cv._footer = render_footer_template(
        cvforge_model.design.templates.footer,
        locale=cvforge_model.locale,
        current_date=cvforge_model.settings._resolved_current_date,
        name=cvforge_model.cv.name,
        single_date_template=cvforge_model.design.templates.single_date,
    ...
    pdf_title_placeholders: dict[str, str] = {
        "CURRENT_DATE": date_object_to_string(
            cvforge_model.settings._resolved_current_date,
            locale=cvforge_model.locale,
            single_date_template=cvforge_model.design.templates.single_date,
        ),
        "NAME": cvforge_model.cv._plain_name or "",
        **build_date_placeholders(
            cvforge_model.settings._resolved_current_date, locale=cvforge_model.locale
        ),
    }
    cvforge_model.settings.pdf_title = substitute_placeholders(
        cvforge_model.settings.pdf_title, pdf_title_placeholders
    )
    ...
    for section in cvforge_model.cv.cvforge_sections:
        ...
        for i, entry in enumerate(section.entries):
            processed_entry = render_entry_templates(
                entry,
                templates=cvforge_model.design.templates,
                locale=cvforge_model.locale,
